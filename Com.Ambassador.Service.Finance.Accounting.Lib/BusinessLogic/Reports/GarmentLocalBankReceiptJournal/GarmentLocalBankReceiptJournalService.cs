...
{
    public class GarmentLocalBankReceiptJournalService : IGarmentLocalBankReceiptJournalService
    {
        ...
        public GarmentLocalBankReceiptJournalService(IServiceProvider serviceProvider, FinanceDbContext dbContext)
        {
        ...
        public List<GarmentLocalBankReceiptJournalViewModel> GetReportData(DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            var Query = GetReportQuery(dateFrom, dateTo, offset);
            return Query.ToList();
        }

        public List<GarmentLocalBankReceiptJournalViewModel> GetReportQuery(DateTime? dateFrom, DateTime? dateTo, int offset)

        {
            DateTime DateFrom = dateFrom == null ? new DateTime(1970, 1, 1) : (DateTime)dateFrom;
            DateTime DateTo = dateTo == null ? DateTime.Now : (DateTime)dateTo;

            List<GarmentLocalBankReceiptJournalViewModel> data = new List<GarmentLocalBankReceiptJournalViewModel>();
            ...
            var Query = from a in _dbContext.GarmentFinanceBankCashReceipts
                        join b in _dbContext.GarmentFinanceBankCashReceiptItems on a.Id equals b.BankCashReceiptId 
                        where a.ReceiptDate.AddHours(7).Date >= DateFrom.Date && a.ReceiptDate.AddHours(7).Date <= DateTo.Date
                        && a.CurrencyRate == 1

                        group new { TotalAmount = b.Summary } by new
                        {
                            a.DebitCoaCode,
                            a.DebitCoaName
                        } into G

                        select new GarmentLocalBankReceiptJournalViewModel
                        {
            ...
            var Query1 = from a in _dbContext.GarmentFinanceBankCashReceipts
                        ...
                        select new GarmentLocalBankReceiptJournalTempViewModel
                        {
            ...
            {
                var debit = new GarmentLocalBankReceiptJournalViewModel
                {
                    Remark = item.Remark,
            ...
            var credit = new GarmentLocalBankReceiptJournalViewModel
            {
                Remark = "       PIUTANG USAHA LOKAL GARMENT",
                Account = "1101.00.4.00",
                Debit = 0,
            ...
            var total = new GarmentLocalBankReceiptJournalViewModel
            {
                Remark = " ",
                Account = "JUMLAH",
        ...
        public MemoryStream GenerateExcel(DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            if (Query.ToArray().Count() == 0)
            {
                ...
                foreach (KeyValuePair<DataTable, String> item in new List<KeyValuePair<DataTable, string>>() { new KeyValuePair<DataTable, string>(result, "Territory") })
                {
                    ...
                    sheet.Cells[$"D5"].Value = ": PENERIMAAN KAS BANK GARMENT LOKAL";
